...
```{r tib_AFC}
tib_spec=tib_spec %>% filter(spec>10)
tib_spec_haute=tib_lemmes %>% 
  filter(lemma %in% tib_spec$lemma )
tib_lex=table(tib_spec_haute$lemma,tib_spec_haute$ministre)
```
...
```{r CA_eigen, fig.width=6, fig.height=5}
library(factoextra)
