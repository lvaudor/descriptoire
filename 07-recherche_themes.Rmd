...
3. pour chaque mot dans chaque document, 
    a.  on tire un thème $k$ en fonction de la distribution calculée à l'étape 1 
    b. en se basant sur ce thème $k$ on tire au hasard un mot à l'aide des valeurs $\beta_{k,p}$ 
...
thematiques
```
Dès lors, on peut essayer d'analyser l'occurrence de ces thématiques en fonction d'une ou plusieurs covariables issues des métadonnées
