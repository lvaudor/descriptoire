...
Il existe plusieurs méthodes et plusieurs algorithmes pour réaliser une recherche de thèmes, mais dans cette partie je me focaliserai sur le *Structural Topic Modelling* (STM) et sur sa mise en oeuvre à l'aide du package R \{stm}.
...
- $E$ est le score correspondant à l'**exclusivité** et est obtenu ********
...
## Communiqués de presse du ministère de l'écologie

Considérons la table suivante, qui résulte d'un travail de récupération, nettoyage, tokenisation, lemmatisation d'un ensemble de communiqués de presse du Ministère de la Transition écologique et de la Cohésion des territoires:

```{r data_ministereco}
tib_docs=readRDS("data/ministereco_tib_docs.RDS")
tib_lemmes=readRDS("data/ministereco_tib_lemmes.RDS") %>% 
  select(doc,date,lemma,ministre)
...
tib_sparse=tib_lemmes %>% 
  ...
  mutate(freq=n()) %>% # ...son nombre d'occurrences puis
  filter(freq>20) %>%  # retire ceux représentés moins de 20 fois dans le corpus
...
```{r get_topicterms}
td_topicterms=tidy(topic_model, matrix="beta") %>% 
  group_by(topic) %>% 
  slice_max(beta,n=15) %>% 
